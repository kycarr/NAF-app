  ...
  [
    {
      ...
      question: "Whatttttt are the two types of electrical communications?",
      answered: false,
      bookmarked: false,
...
router.post('/api/login', (req,res)=>{
    ...
    //will need to fetch the first and last name based on user id based on username
    let firstName = "Yubo";
    let lastName = "Wang";
...
router.get('/api/questionAnswerStore', (req,res) => {
  let userId = req.query.userId;
  ...
  console.log(answer);
  res.status(200).end('Question details received');
...
router.post('/api/questionResponseStore', (req,res) => {
  let userId = req.body.userId;
  let sectionId = req.body.sectionId;
  ...
  console.log("sectionId: " + sectionId);
  console.log(answer);
  res.status(200).end('Success fron questionResponseStore');

});

router.post('/api/finishTest', (req, res) => {
  let userId = req.body.userId;
